...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
                                                ...
                                                <asp:UpdatePanel ID="UpdatePanel1" runat="server">
                                                    <ContentTemplate>
                                                        <div class="input-icon input-icon-sm right">
                                                            ...
                                                            <asp:TextBox runat="server" ID="txtCompletionPercentage" oninput="validatePercentage(this)" MaxLength="3" ClientIDMode="Static" class="form-control input-sm" autocomplete="off" placeholder=""></asp:TextBox>
                                                ...
                                                <ContentTemplate>
                                                    <div class="circle-sec">
                                                        <section>
                                                            ...
                                                            <div class="pie" id="progressPie" data-pie='{ "lineargradient": ["green"], "round": true, "percent": 0,"colorCircle": "#e6e6e6" }'></div>
                                                        </section>
                                                    </div>
                                                </ContentTemplate>
                                                <Triggers>
                        ...
                        <div class="card-footer">
                            <div class="d-flex justify-content-center">
                                <asp:Button
                                    ...
                                    Text="Update" ValidationGroup="projval"
    ...
    <script>
        function validatePercentage(input) {
            ...
            if (value > 100) {
                input.value = 100; // Set max value to 100
            } else {
                input.value = value; // Allow numbers within range
            }
        }
</script>
    ...
    <script>
        function initializeCircularProgressBar() {
            const pie = document.querySelectorAll(".pie");
            const range = document.querySelector('#txtCompletionPercentage'); // Get the textbox

            if (!pie.length || !range) return;

            const circle = new CircularProgressBar("pie");
            circle.initial();

            // Get the current textbox value and apply it to the circle
            const currentValue = range.value.trim(); // Ensure we get the correct value
            if (currentValue) {
                pie.forEach((el, index) => {
                    const options = {
                        index: index + 1,
                        percent: parseInt(currentValue, 10) || 0 // Convert to number
                    };
                    circle.animationTo(options);
                });
            }

            // Update circle when textbox value changes
        ...
        // Reinitialize after UpdatePanel update
