...
public partial class adminkey2hcomaddprojectprogress : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else if (hdnProgressID.Value != "" && hdnProgressID.Value != null)
            {
            ...
            {
                if (!string.IsNullOrEmpty(hdnProgressID.Value))
                {
                    DataTable dt = KPP.ViewprojectImageByprogressID(Convert.ToInt32(hdnProgressID.Value));
            ...
            else
            {
                string script1 = string.Format(
                    "Swal.fire({{ title: '{0}', customClass: {{ confirmButton: 'handle-btn-success' }} }});",
                    "hur wr"
                );
        ...
        {
            CI.StoreExceptionMessage("add-project-progress.aspx", "btnsubmit", ex.Message, "Not Fixed");
        }
    }
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("view-project-progress.aspx");
        }
    }
    ...
    public int AddData()
    {
        ...
        try
        {
            if (!Alreadyexistprojectprogress(Convert.ToInt32(ddlprojects.SelectedValue), ddlmonth.SelectedValue, Convert.ToInt32(ddlyear.SelectedValue)))
            {
                ...
                hdnProgressID.Value = Convert.ToString(KPP.AddProjrctProgress(KPP));
                if (Convert.ToInt32(hdnProgressID.Value) != 0)
                {
                    BindThumnailimage();
                    ret = AddProjectimage();
                }
        ...
        if (!string.IsNullOrEmpty(ddlprojects.SelectedValue) && !string.IsNullOrEmpty(ddlmonth.SelectedValue) && !string.IsNullOrEmpty(ddlyear.SelectedValue))
        {
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {
                        hdnProgressID.Value = Convert.ToString(dt.Rows[0]["ProgressID"]);
                    }
                }
                else
                {
    ...
    public void BindProjectImageList()
    {
        ...
        try
        {
            if (!string.IsNullOrEmpty(hdnProgressID.Value))
            {
                id = Convert.ToInt32(hdnProgressID.Value);
            }
            else
            {
                id = ProgressID;
            ...
            DataTable dt = KPP.ViewprojectImageByprogressID(id);
            if (dt != null && dt.Rows.Count > 0)
            {
