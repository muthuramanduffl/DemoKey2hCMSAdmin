    ...
    protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "btnUpdateCost")
        {
            try
            {
                ...
                Response.Redirect("add-cost-details1.aspx?FlatID=" + ID +"&&Source=All", false);
        ...
        if (e.CommandName == "UpdateShedule")
        {
            ...
            {
                int ID = Convert.ToInt32(e.CommandArgument);
                Response.Redirect("add-payment-schedule1.aspx?FlatID=" + ID + "&&Source=All", false);
                HttpContext.Current.ApplicationInstance.CompleteRequest();
            }
            catch (Exception ex)
            {
        ...
        if (e.CommandName == "UpdateDemand")
        {
            ...
            {
                int ID = Convert.ToInt32(e.CommandArgument);
                Response.Redirect("add-demand1.aspx?FlatID=" + ID + "&&Source=All", false);
                HttpContext.Current.ApplicationInstance.CompleteRequest();
            }
            catch (Exception ex)
            {
        ...
        if (e.CommandName == "UpdateUpdateTransaction")
        {
            ...
            {
                int ID = Convert.ToInt32(e.CommandArgument);
                Response.Redirect("add-payment-transaction1.aspx?FlatID=" + ID + "&&Source=All", false);
                HttpContext.Current.ApplicationInstance.CompleteRequest();
            }
            catch (Exception ex)
            {
