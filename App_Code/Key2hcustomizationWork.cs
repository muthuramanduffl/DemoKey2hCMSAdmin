...
public class Key2hcustomizationWork
{
    ...
    public int ExistworkbyID(string ID,string AddedBy)
    {
        DataTable dt = new DataTable();
        string connetionString = null;
        SqlConnection cnn;
        ...
        try
        {
            using (SqlCommand command = new SqlCommand("ExistworkbyID", cnn))
            {
                cnn.Open();
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.Add(new SqlParameter("@ID", string.IsNullOrWhiteSpace(ID) ? (object)DBNull.Value : Convert.ToInt32(ID)));
    ...
    public int DeleteAllCustomisationWorksByFlatIDAndAddedBy(int FlatID, string AddedBy)
    {
        ...
        try
        {
            using (SqlCommand command = new SqlCommand("DeleteAllCustomisationWorksByFlatIDAndAddedBy", cnn))
            {
                cnn.Open();
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.Add(new SqlParameter("@FlatID", FlatID));
