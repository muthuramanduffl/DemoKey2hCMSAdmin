...
public class Key2hProjectbroadcast
{
    ...
    public int AddBroadcastToFlats(Key2hProjectbroadcast K2)
    {
        string connetionString = null;
        SqlConnection cnn;
        connetionString = GetSqlConnection();
        ...
        try
        {
            using (SqlCommand command = new SqlCommand("AddBroadcastToFlats", cnn))
            {
                cnn.Open();
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.Add(new SqlParameter("@ProjectID", K2.ProjectID));
    ...
    public DataTable ViewAllNotifications(string prID)
    {
        string connectionString = GetSqlConnection();
        SqlConnection cnn = new SqlConnection(connectionString);
        DataTable dt = new DataTable();
        ...
        try
        {
            using (SqlCommand command = new SqlCommand("ViewAllNotifications", cnn))
            {
                cnn.Open();
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.Add(new SqlParameter("@ProjectID", string.IsNullOrWhiteSpace(prID) ? (object)DBNull.Value : Convert.ToInt32(prID)));
