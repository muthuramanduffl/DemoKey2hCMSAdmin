    ...
    public DataTable ViewAllBlock(string ID, string status, string prID)
{
    ...
    try
    {
        using (SqlConnection cnn = new SqlConnection(connectionString)) // Properly dispose the connection
        {
            using (SqlCommand command = new SqlCommand("ViewAllBlock", cnn))
            {
                ...
                {
                    command.Parameters.Add(new SqlParameter("@BlockStatus", status == "1" ? Convert.ToBoolean(true) : Convert.ToBoolean(false)));
                }
    ...
    {
        CE.StoreExceptionMessage("Class file", "ViewAllBlock", ex.Message, "Not Fixed");
    }
