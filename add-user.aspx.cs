...
public partial class adminkey2hcom_AddRM : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            if (Request.QueryString["RMID"] != null)
            {
                ...
                else
                {
                    ...
                    window.location.href = 'view-users.aspx';}}); </script>");
    ...
    public void Bind(int id)
    {
        ...
        {
            CI.StoreExceptionMessage("add-user.aspx", "Bind", ex.Message, "Not Fixed");
        }
    }
            ...
            else
            {
                ...
                if (ret == 1)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                        ...
                        "    } " +
                        "}).then((result) => { " +
                        "    window.location.href = 'view-users.aspx'; " +
                        "});", true);
                }
                else
                {
                    string script = string.Format(@"<script>
                        Swal.fire({{
                            title: 'User details couldn't be updated due to a server or network issue. Please try again in some time!', 
                            confirmButtonText: 'OK', 
                            customClass: {{ confirmButton: 'handle-btn-success' }} 
                        }}).then((result) => {{ 
    ...
    public int AddRMDetails()
    {
        ...
        {
            CI.StoreExceptionMessage("add-user.aspx", "AddRMDetails", ex.Message, "Not Fixed");
        }
        return ret;
    }
    ...
    public int UpdateRMDetails()
    {
        ...
        {
            CI.StoreExceptionMessage("add-user.aspx", "UpdateRMDetails", ex.Message, "Not Fixed");
        }
        return ret;
    }
