...
public partial class viewamenities : System.Web.UI.Page
{
    ...
    Key2hAmenities KAM = new Key2hAmenities();
    ...
    public string Bindprojectname(int Prid)
    {
        ...
        {
            CE.StoreExceptionMessage("view-amenities.aspx", "Bindprojectname", ex.Message, "Not Fixed");
        }
        return strname;
    }
    public void Bind()
    ...
    public DataTable Get()
    {
        ...
        try
        {
            dt = KAM.ViewAllAmenitiesByFilter("", "", Userid);
        }
        catch (Exception ex)
        {
            CE.StoreExceptionMessage("view-amenities.aspx", "Get", ex.Message, "Not Fixed");
        }
        return dt;
    }
    ...
    protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            try
            {
                ...
                Response.Redirect("add-amenities.aspx?ID=" + ID, false);
        ...
        else if (e.CommandName == "Delete")
        {
            try
            {
                int ID = Convert.ToInt32(e.CommandArgument);
                int ret = 0;
                ret = KAM.DeleteAmenitiesByProjectID(ID,Userid);
                if (ret > 0)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                       "Swal.fire({ " +
                       "  title: 'Amenities details has been deleted ', " +
