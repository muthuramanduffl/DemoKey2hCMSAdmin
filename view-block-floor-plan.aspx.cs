...
public partial class view_flat_floor_plan : System.Web.UI.Page
{
    ...
    public string Bindprojectname(int Prid)
    {
        ...
        {
            CE.StoreExceptionMessage("view-block-floor-plan.aspx", "Bindprojectname", ex.Message, "Not Fixed");
        }
        return strname;
    }
    public void Bind()
    ...
    public DataTable Get()
    {
        ...
        try
        {
            dt = KBF.ViewBlockPlansbyFilter("","","");
        }
        catch (Exception ex)
        {
            CE.StoreExceptionMessage("view-block-floor-plan.aspx", "Get", ex.Message, "Not Fixed");
        }
        return dt;
    }
    ...
    public string BindBlockname(int ID)
    {
        ...
        {
            CE.StoreExceptionMessage("view-flat-booking.aspx", "BindBlockname", ex.Message, "Not Fixed");
        }
        return Block;
    }
    public string ViewFlatNameByFlatID(int ID)
    {
        ...
        {
            CE.StoreExceptionMessage("view-flat-booking.aspx", "ViewFlatNameByFlatID", ex.Message, "Not Fixed");
        }
        return Block;
    }
    ...
    protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            try
            {
                ...
                Response.Redirect("add-block-floor-plan.aspx?ID=" + ID, false);
        ...
        else if (e.CommandName == "Delete")
        {
            try
            {
                int ID = Convert.ToInt32(e.CommandArgument); 
                int ret = 0; 
                ret = KBF.DeleteBlockFloorPlanbyBlockID(ID);
                if (ret > 0)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                       "Swal.fire({ " +
                       "  title: 'Block Floor plan details has been deleted ', " +
                ...
                else
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                      "Swal.fire({ " +
                      "  title: 'Block Floor plan details has been not deleted ', " +
                      "  confirmButtonText: 'OK', " +
                      "  customClass: { " +
                      "    confirmButton: 'handle-btn-success' " + "  } " +
                      "}).then((result) => { " +
