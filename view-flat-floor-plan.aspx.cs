...
public partial class view_flat_floor_plan : System.Web.UI.Page
{
    ...
    Key2hFlatFloorPlan KFFP = new Key2hFlatFloorPlan();
    ...
    public string Bindprojectname(int Prid)
    {
        ...
        {
            CE.StoreExceptionMessage("view-floor-plan.aspx", "Bindprojectname", ex.Message, "Not Fixed");
        }
        return strname;
    }
    public void Bind()
    ...
    public DataTable Get()
    {
        ...
        try
        {
            dt = KFFP.ViewflatFloorPlansbyFilter("","","","");
        }
        catch (Exception ex)
        {
            CE.StoreExceptionMessage("view-floor-plan.aspx", "Get", ex.Message, "Not Fixed");
        }
        return dt;
    }
    ...
    protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            try
            {
                ...
                Response.Redirect("add-flat-floor-plan.aspx?ID=" + ID, false);
        ...
        else if (e.CommandName == "Delete")
        {
            try
            {
                int ID = Convert.ToInt32(e.CommandArgument); 
                int ret = 0; 
                ret = KFP.DeleteflatFloorPlanbyFlatID(ID);
                if (ret > 0)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                       "Swal.fire({ " +
                       "  title: 'Flat Floor plan details has been deleted ', " +
