...
public partial class viewprojectqualityreports : System.Web.UI.Page
{
    ...
    public string Bindprojectname(int Prid)
    {
        ...
        {
            CE.StoreExceptionMessage("View-project-quality-reports.aspx", "Bindprojectname", ex.Message, "Not Fixed");
        }
        return strname;
    }
    public void Bind()
    ...
    public DataTable Get() 
    {
        ...
        try
        {
            dt = KQR.ViewAllProjectQP();
        }
        catch (Exception ex)
        {
            CE.StoreExceptionMessage("View-project-quality-reports.aspx", "Get", ex.Message, "Not Fixed");
        }
        return dt;
    }
    ...
    protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            try
            {
                ...
                Response.Redirect("add-quality-reports.aspx?ID=" + ID, false);
        ...
        else if (e.CommandName == "Delete")
        {
            try
            {
                int ID = Convert.ToInt32(e.CommandArgument);
                int ret = 0;
                ret = KQR.DeleteProjectQPbyProjectID(ID);
                if (ret > 0)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                       "Swal.fire({ " +
                       "  title: 'Project quality report details has been deleted ', " +
