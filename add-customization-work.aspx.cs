...
public partial class adminkey2hcom_AddCustomizationWork : System.Web.UI.Page
{
    ...
    public void BindWorkprogress()
    {
        if (!string.IsNullOrEmpty(ddlFlatNumber.SelectedValue))
        {
            DataTable dt = KCW.ViewCustomisationWorkApprovalBystatus(ddlFlatNumber.SelectedValue, Userid, "Approved");
            if (dt != null && dt.Rows.Count > 0)
            {
                ddlWorkList.DataSource = dt;
                ddlWorkList.DataTextField = "CustomizationWork";
                ddlWorkList.DataValueField = "CWAID";
    ...
    protected void ddlWorkList_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(ddlFlatNumber.SelectedValue))
        {
            ...
            txtAmount.Text = "";
            DataTable dt = KCW.ViewCustomisationWorkApprovalAmount(ddlFlatNumber.SelectedValue, Userid, "Approved", ddlWorkList.SelectedValue);
            if (dt != null && dt.Rows.Count > 0)
            {
    ...
    public static string DeleteCustomizationWorksTitle(string CWID, string selectedValue)
    {
        ...
        {
            CE.StoreExceptionMessage("add-customization-work.aspx", "DeleteCustomizationWorksTitle", ex.Message, "Not Fixed");
        }
        return strerror;
    }
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        if (string.IsNullOrEmpty(ddlProName.SelectedValue) && string.IsNullOrEmpty(ddlBlockNumber.SelectedValue) &&
            ...
            && string.IsNullOrEmpty(ddlWorkList.SelectedValue) && string.IsNullOrEmpty(ddlWorkProgress.SelectedValue)
        ...
        if (string.IsNullOrEmpty(labelerror))
        {
            if (string.IsNullOrEmpty(lblcustomernameerror.Text))
            {
                ...
                {
                    if (KCZW.ExistworkbyID(ddlWorkList.SelectedValue, Userid) == 0)
                    {
                        int ret = 0;
                        ret = AddData();
                        if (ret == 1)
                    ...
                    else
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                         "Swal.fire({ text: '" + "Work status Already exist" + "', customClass: { confirmButton: 'handle-btn-success' } });", true);
                        BindCustomizationWorkList();
                    }
                }
                else
            ...
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                           "Swal.fire({  text: '" + lblcustomernameerror.Text + "', customClass: { confirmButton: 'handle-btn-success' } });", true);
        ...
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                          "Swal.fire({ text: '" + labelerror + "', customClass: { confirmButton: 'handle-btn-success' } });", true);
    ...
    public int AddData()
    {
        ...
        try
        {
            ...
            KCZW.strWorkTitle = ddlWorkList.SelectedValue;
